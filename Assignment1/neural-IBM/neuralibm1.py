...
#     pa_x = ?  # Shape: [B, 1, M]
    pa_x = tf.reshape(pa_x, [batch_size, 1, longest_x])
    print('pa_x', pa_x.shape)
    

    # Now we perform the tiling:
    # pa_x = ?  # [B, N, M]
    pa_x = tf.tile(pa_x, [1, longest_y, 1])
    print(pa_x)
...
#     print(batch_size, longest_x, self.emb_dim)
    # Here we apply the MLP to our input.
    
    print('lalalala')
    print('mlp', mlp_input.shape)
    print('w', self.W_eh.shape)
